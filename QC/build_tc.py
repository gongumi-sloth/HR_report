...
HEADERS = ["담당","TC ID","대분류","중분류","우선\n순위","테스트 시나리오(케이스명)","사전조건",
           "테스트 절차","입력/조건","기대결과",
           "QC결과","실행일","QC담당자",
           "결함 내용","심각도","개발 수정상태","재확인(QC)","비고","구분","키(작업자)"]
# 그룹별 헤더색 (맨 앞 '담당' 포함 TC정의 10칸 / 끝에 비고·구분·키)
HCOLORS = [C_TCDEF]*10 + [C_QC]*3 + [C_DEFECT]*4 + [C_ETC]*3
WIDTHS  = [12,10,14,16,7,30,22,30,18,40, 9,11,10, 28,9,13,12,16,10, 9]

# 컬럼 인덱스(1-base) — 맨 앞 '담당'(VLOOKUP) / 끝에 '구분'·'키(작업자N)'
COL_TCID=2; COL_PRIO=5; COL_QC=11; COL_SEV=15; COL_FIX=16; COL_RECHK=17; COL_GUBUN=19; COL_KEY=20
...
SCEN_OWNER={  # 시나리오-의존 케이스(케이스명 앞 코드) → 담당 작업자. 사람만 바꿔 끼우면 됨
 ...
 "E1":1,"E2":1,"E3":1,"E4":1,"E5":1,"E6":1,"E7":4,"E8":1,
...
# 담당자 매핑(작업자N → 이름). 이름만 여기서 바꾸면 시나리오 시트 매핑표·담당·합계에 자동 반영.
# (TC 시트 [담당]은 작업자N 코드 유지 = 고정 매칭 키)
WORKER_NAMES={1:"조아영",2:"기효선1",3:"김재현",4:"기효선2",5:"최고운",6:"전예원",7:"윤경은"}
...
("E. 합불 배지","P1","E1 직군 위치 배지","직무별 비교군 10명 이상일 때만 출력. 텍스트='{benchmarks.group} 상위 {benchmarks.percentile}%'. 비교군 10명 미만(9명 이하)이면 상위% 배지·수치 미출력(접수화면 안내문과 동일 기준, E8 연계)","R"),
...
("E. 합불 배지","P1","E4 합불 배지 색 분기","'N배수 이내'→.fill(파랑 #e0f3ff), 'N배수 미만'→.fill.over(주황 #d75d00). 텍스트·색 클래스 동시 일치(점수기준 제거)","R"),
...
("E. 합불 배지","P1","E7 종합결과 페이지 합불 칩","복수 라이선스 종합결과 페이지 상단에 배수 기준 판정 칩이 단독과 동일 규칙으로 표시('N배수 이내' 파랑, 'N배수 미만' 주황). 텍스트·색 모두 일치","R"),
("E. 합불 배지","P1","E8 비교군 10명 미만 상위% 미출력","직무별 비교군 인원이 10명 미만(9명 이하)이면 직군 위치 배지(상위%)·요약막대 상위10% 위치점(.dot-top)·'상위 10%' 범례·미니막대 top10 점이 모두 미출력. 10명 이상이면 출력. 평균(avg) 비교는 인원과 무관하게 유지(접수화면 안내문 기준)","R"),
("F. 검증 포인트","P2","F1 강점 카드","강점=factor 중 상위 등급 2개(verification.strengths)와 개수·순서 일치, 배지 .vp-badge.up. 답변적합도·구체성·본인소개(metric)는 강점에 미노출(factor만)","R"),
("F. 검증 포인트","P2","F2 검토 카드","검토=factor 중 하위 등급 2개(verification.reviews)와 개수·순서 일치, 배지 .vp-badge.down. 답변적합도·구체성·본인소개(metric)는 검토에 미노출(factor만)","R"),
...
("F. 검증 포인트","P2","F6 근거요약·면접질문",".vp-sum=각 factor 근거요약(해당 항목 근거문장으로 추출된 문장 요약, 근거요약 API 활용). .vp-tip .ttxt=면접질문(해당 항목 근거문장 기반 추천 질문)","D"),
("F. 검증 포인트","P2","F7 하단 하드스킬 칩","강점카드 하단=확인된 하드스킬(검출), 검토카드 하단=확인되지 않은 하드스킬(미검출)","R"),
("F. 검증 포인트","P1","F8 강점/검토 후보=factor 한정","강점/검토 후보 = factor만 — 직무적합도(직무경험·직무지식·직무동기) / 직무역량 각 역량분자 / 조직적합도 각 역량분자 / 지원동기(직무동기·회사동기). 답변적합도·구체성·본인소개는 metric이라 강점/검토 영역에 미노출(SDS 3-5 변경)","R"),
("F. 검증 포인트","P2","F9 강점/검토 선정 우선순위","동일 등급 다수일 때 ① 가중치 높은 factor 우선, ② 동일 등급·동일 가중치면 정해진 순서: 직무역량 역량분자 > 직무경험 > 직무지식 > 직무동기(직무적합도) > 조직적합도 역량분자 > 직무동기(지원동기) > 회사동기","R"),
...
("S. 전형 통과 여부","P1","S1 전형 통과 여부 텍스트","'적격/부적격' 대신 '전형 통과 기준'(배수 기준) 텍스트로 표기(AI기본법 표현 준수). 점수 기준 제거됨","S"),
("S. 전형 통과 여부","P1","S2 상위 n% 표기","BP평가 점수에 직군 내 상위 n%가 동반 표기. 단, 직무별 비교군 10명 이상일 때만(10명 미만 미표기 — 접수화면 안내 기준, E8 연계)","R"),
("S. 전형 통과 여부","P1","S3 배수 기준 결과지 표기","접수 '배수 기준' 설정 시: 점수 순위가 (모집인원×배수) 이내면 'N배수 이내'(권장·파랑), 미달이면 'N배수 미만'(검토 필요·주황) 기재(SDS BP콘텐츠상세 3-1)","R"),
("S. 전형 통과 여부","P1","S4 BP 점수 기준 미사용(배수 단일)","접수화면에서 BP평가 전형 통과 기준은 '배수 기준' 단일 — '점수 기준' 옵션 제거됨(점수 기준은 RP매칭 부적격 기준에만 존재). 결과지에 BP 점수기준 기반 표기 케이스 없음","R"),
("S. 전형 통과 여부","P1","S5 전형통과 판정 출처·위치","기준 점수=단독검사 시 BP(종합)점수, 복수 라이선스 시 '프리즘 종합점수'. 단독→BP결과지에 배수 판정 표기. 복수→종합결과 페이지 상단에만 배수 판정 표기(BP 결과지엔 전형통과 표현 없이 직무 상위 n%만 표시). 접수 배수 기준 미체크(미사용) 시 미표기","R"),
...
("V. 타 라이선스 페이지","P2","V2 합불 표현 워딩 통일","BP·종합결과의 전형통과 표현은 'N배수 이내/미만' 칩으로 표기되고 BP 관련 페이지에 '적격/부적격' 직접표현 없음(AI기본법). 단, RP매칭 부적격 기준 결과 표현은 '부적격' 유지(별개 항목)","S"),
...
("접수-등록 자소서 유형","P1","비교군 10명 미만 상위% 안내문","접수 화면 - 등록 자소서 유형(벌크형/단건형) 선택","벌크형/단건형 설명 하단 안내문 확인","-","'직무별 평가 비교군이 10명 미만일 경우 BP평가에서 상위 10% 비교 정보가 제공되지 않습니다' 안내문이 벌크형/단건형 설명 하단에 노출(결과지 E1·E8·S2와 동일 기준)"),
("접수-BP평가 버전","P2","BP v2 라디오 제거(v3/v4만)","접수 화면 - BP평가 버전 선택","버전 선택지 확인","-","BP평가 버전 선택에서 v2 라디오 제거, v3·v4만 노출"),
("접수-BP평가 버전","P1","v3 선택 시 직무기술서 미출력","접수 화면 - BP평가 v3/v4","버전 v3↔v4 전환","-","BP평가 v3 선택 시 직무기술서 첨부 영역 미출력, v4 선택 시 출력"),
("접수-전형 통과 기준","P1","배수 기준 단일화(점수 기준 제거)","프리즘워크/브릿지 접수 화면","전형 통과 기준 영역 확인","-","전형 통과 기준이 '배수 기준' 단일로 운영. 기존 '점수 기준' 옵션 제거(점수 기준은 RP매칭 부적격에만 존재). 프리즘 종합점수 기준 산출(BP 단독 선택 시 BP평가 점수 기준)"),
("접수-전형 통과 기준","P1","사용 여부 라디오 제거·기본 미체크","접수 화면","사용 안 함/사용함 라디오 확인","-","'사용 안 함/사용함' 라디오 제거. 배수 기준 체크박스 1개로 사용 여부 결정(기본값 미체크, 체크 시 적용·입력칸 활성화, 미체크 시 미적용·입력칸 비활성화)"),
("접수-전형 통과 기준","P2","전형 통과 기준 별도 영역 분리","접수 화면","영역 위치 확인","-","전형 통과 기준이 별도 영역으로 분리되어 직무기술서 첨부 영역 위에 배치(BP평가 결과 기준 판정)"),
("접수-전형 통과 기준","P1","배수 기준 입력·검증","접수 화면","배수 기준 체크 후 입력·저장","모집인원·배수 미입력 / 문자·기호 입력","체크 시 '모집인원 ( )명 / ( )배수 미만 시 검토 필요' 활성화. 모집인원·배수 모두 입력 필수(미입력 저장 차단), 숫자만 입력(문자·기호 불가). 결과지 표기: 충족 'N배수 이내'(권장)/미달 'N배수 미만'(검토 필요)"),
("접수-전형 통과 기준","P2","복수 평가 판정 안내문","접수 화면 · BP 포함 2종 이상 선택","안내문 노출 확인","-","BP평가 포함 2개 이상 평가 선택 시 'BP평가를 포함해 2개 이상의 평가를 선택하여, 모든 평가 결과를 기준으로 판정합니다' 안내문 노출(1종 선택 시 미노출)"),
("접수-직무기술서 업로드","P1","활성화 조건·필수","접수 화면","BP/RP 선택·해제","-","BP평가 또는 RP매칭 선택 시 직무기술서 업로드 영역 활성화(하나만 선택해도). 단 BP v3 단독 선택 시 미출력. 필수 업로드(미업로드 시 제출 불가). RP매칭 영역과 동일 테이블 형태"),
("접수-직무기술서 업로드","P1","파일 추가·목록 표시","직무기술서 영역 활성","[파일 추가] 클릭·업로드","JD/채용공고 파일","[파일 추가] 시 파일 탐색기. 업로드 전 '등록된 파일이 없습니다.' 표시 / 업로드 후 파일명·파일크기·삭제 버튼 표시"),
("접수-직무기술서 업로드","P1","용량·중복 제한","직무기술서 영역","파일 업로드","3MB 초과·총 10MB 초과·동일 파일명","파일당 최대 3MB·전체 10MB 제한 초과 시 차단. 동일 파일명 중복 업로드 시 모달 출력(중복 불가)"),
("접수-직무기술서 업로드","P2","허용 포맷·안내 메시지","직무기술서 영역","안내 메시지·포맷 확인","-","안내: '하드스킬 평가(BP평가)·직무 매칭(RP매칭)을 위해 JD 또는 채용공고 파일을 등록해주세요'. 허용 PDF·hwp·hwpx·docx·doc / 파일당 3MB·총 10MB·동일 파일명 중복 불가"),
("접수-직무기술서 업로드","P2","선택 해제 후 제출 시 파일 소실","BP/RP 선택+직무기술서 업로드 상태","BP/RP 선택 해제 후 검사요청서 제출","-","BP평가·RP매칭 선택·업로드 후 다시 선택 해제하고 제출하면 업로드한 직무기술서 파일은 소실됨"),
("접수-RP매칭 부적격 기준","P1","점수 기준 단일 체크박스화","프리즘워크 접수 화면","RP매칭 부적격 기준 UI 확인","-","BP 전형통과 기준과 동일 방식. '사용 안 함' 라디오 제거, '점수 기준' 단일 체크박스(기본값 미체크, 체크 시 적용·입력칸 활성화, 미체크 시 미적용·비활성화)"),
("접수-RP매칭 부적격 기준","P1","점수 입력·검증","RP 점수 기준 체크","점수 입력","0~100 외 / 문자·기호","'( )점 미만 부적격' 입력, 0~100 사이 수만·숫자만 입력(문자·기호 불가). 임계치 표현 '( )점 미만' 유지, 결과 표현 '부적격' 유지"),
("접수-RP매칭 부적격 기준","P2","프리즘브릿지 컨트롤 없음","프리즘브릿지 접수 화면","RP매칭 부적격 기준 확인","-","프리즘브릿지의 RP매칭은 안내 문구만 제공 → 부적격 기준 컨트롤 없음"),
...
def write_sheet(title,prefix,rows):
    ...
    for i,r in enumerate(rows,1):
        ...
        if gubun=="시나리오":
            key=f"작업자{SCEN_OWNER.get(code,1)}"
        else:
            key=f"작업자{_common_ctr[0]%WORKERS+1}"; _common_ctr[0]+=1
        rr=i+1
        # 담당(A) = 작업자N(T열 키)로 시나리오 [담당](G)열을 조회 → 시나리오 이름만 바꾸면 TC도 일괄 반영
        dam=f"=IFERROR(INDEX('시나리오'!$G$5:$G$11,MATCH($T{rr},'시나리오'!$L$5:$L$11,0)),$T{rr})"
        # 컬럼: 1=담당(수식) / 2=TCID / 3-10=TC정의 / 11-17=QC·결함 / 18=비고 / 19=구분 / 20=키(작업자N)
        vals=[dam]+[tcid]+r[:8]+[""]*7+[r[8]]+[gubun]+[key]
        for c,v in enumerate(vals,1):
            cell=ws.cell(rr,c,v)
            cell.font=F(size=9)
            cell.alignment=Alignment(vertical="top",wrap_text=True,
                horizontal="center" if c in (1,COL_TCID,COL_PRIO,COL_QC,COL_SEV,COL_FIX,COL_RECHK,COL_GUBUN,COL_KEY) else "left")
            cell.border=BORDER
            if i%2==0:
...
CHANGES = [
("접수","전형 통과 기준 '배수 기준' 단일화(점수 기준 제거)·사용여부 라디오 제거·별도 영역 분리(직무기술서 위) · 비교군 10명 미만 상위% 안내문","프리즘 접수 화면 기능 개발"),
("접수","BP 버전 v2 라디오 제거(v3/v4만)·v3 선택 시 직무기술서 영역 미출력","프리즘 접수 화면 기능 개발"),
("접수","직무기술서 업로드(필수·테이블 UI·3MB/10MB·중복불가·선택해제 시 소실, RP매칭과 공유)","프리즘 접수 화면 기능 개발"),
("접수","RP매칭 부적격 기준 UI 변경(사용안함 라디오 제거·'점수 기준' 단일 체크박스, '점미만'·'부적격' 유지)","프리즘 접수 화면 기능 개발"),
...
SCENARIOS = [
("SC-1 단독·배수권장","BP 단독","배수기준 → 권장(이내)","OFF","B 정수민","단독 구성(W1·Q1·Q2) · 배수기준 권장칩(S3·E) · 통합본문 OFF=개별본문(X3)"),
("SC-2 단독·배수검토필요","BP 단독","배수기준 → 검토 필요(미달)","OFF","C 김도윤","단독 구성(W1) · 배수기준 검토필요 주황칩(S3·E4)"),
("SC-3 단독·통과기준 미사용","BP 단독","사용 안 함(배수 미체크)","OFF","A 홍길동","단독 구성(W1) · 전형통과 미표기(S5) · 비교군 10명 미만 상위% 미출력(E8)"),
...
def build_scenario_sheet():
    ...
    ws.cell(2,1,"담당자 이름은 이 시트 [담당](G)열에서만 관리 → TC 3종 [담당]이 INDEX/MATCH로 같은 이름을 자동으로 가져옴. 분배/완료는 작업자N 키(L열 ↔ TC 키열)로 자동 집계.").font=F(size=10,color="595959")
    hdr=["시나리오","라이선스 조합","전형통과 기준","통합본문","샘플 자소서","적용 TC(주)","담당","분배","완료","남은","상태","키(작업자)"]
    widths=[19,20,15,8,12,28,9,7,7,7,9,10]
    hrow=4
    for c,(h,w) in enumerate(zip(hdr,widths),1):
        cell=ws.cell(hrow,c,h); cell.font=F(bold=True,color="FFFFFF",size=10)
        cell.fill=PatternFill("solid",fgColor=C_TCDEF); cell.alignment=Alignment(horizontal="center",vertical="center",wrap_text=True); cell.border=BORDER
        ws.column_dimensions[get_column_letter(c)].width=w
    ws.row_dimensions[hrow].height=24
    ws.cell(3,1,"※ 담당자 이름은 이 시트 [담당](G)열에서만 수정하세요 → TC 3종 [담당]이 자동으로 같은 이름을 가져옵니다. (분배/완료는 G 이름이 아니라 L열 작업자N 키로 집계)").font=F(size=9,color="B5482A")
    SNS3=("TC_결과지","TC_API","TC_어드민")
    for i,row in enumerate(SCENARIOS,1):
        r=hrow+i
        bun="+".join([f"COUNTIF('{s}'!$T$2:$T$2000,$L{r})" for s in SNS3])
        done="+".join([f"COUNTIFS('{s}'!$T$2:$T$2000,$L{r},'{s}'!$K$2:$K$2000,\"<>\")" for s in SNS3])
        vals=list(row)+[WORKER_NAMES.get(i,f"작업자{i}"),f"={bun}",f"={done}",f"=H{r}-I{r}","",f"작업자{i}"]
        for c,v in enumerate(vals,1):
            cell=ws.cell(r,c,v); cell.font=F(size=10)
            cell.alignment=Alignment(vertical="center",wrap_text=True,horizontal="center" if c in (4,5,7,8,9,10,11,12) else "left"); cell.border=BORDER
    ...
    n=tr+2
    ws.cell(n,1,"※ 검수자: 본인 시트에서 [담당]을 본인 이름으로 필터 → 내 TC만(공통 일부 + 내 시나리오). [구분]으로 공통/시나리오 구분.").font=F(size=9,color="595959")
    ws.cell(n+1,1,f"※ 분배 합계 = {n_rpt+n_api+n_adm}(전 TC 배정). 완료 = QC결과 입력된 행. 담당자가 바뀌면 이 시트 [담당](G)열 이름만 수정 → TC 3종 [담당] 자동 반영(분배/완료는 작업자N 키로 집계되어 영향 없음).").font=F(size=9,color="595959")
    ws.freeze_panes=f"A{hrow+1}"
...
start=["1) 내 담당 시트를 열고 [담당]을 본인 이름으로 필터 → 내 TC만 나옵니다(공통 일부 + 내 시나리오). 전부 볼 필요 없음.",
...
wb.active=0
import os
out=os.path.join(os.path.dirname(os.path.abspath(__file__)),"프리즘BP-4.2-테스트케이스-20260624.xlsx")
wb.save(out)
print("저장:",out)
