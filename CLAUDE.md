...
- ② 진단 신뢰도 영역 (①과 같은 흰 셀 안, 분할선 없음 / 패딩 20px 32px / gap 24px): bg `linear-gradient(270deg, #EEF6FF 0%, #FFF 100%)` — **몬×역은 이 영역 흰 배경, 그라데이션을 ③ 교차 유형 카드에 적용**(2026-08-03)
...
- **역량 카드 (액센트형, 2026-08-03 확정 — 구 카드형 1px #C6C6C6·r8 폐기)**: 보더·radius 없이 **좌측 액센트 2px #E3E3E3** / padding 4px 8px 8px 16px / 카드 간 28px / 핵심역량 카드 bg rgba(255,243,219,0.3) **radius 0** / 헤더 = 역량명 Bold 18 + 핵심역량 태그(이름 옆) + **점선 리더**(flex 1 · border-bottom 1px dotted #C6C6C6 · margin 0 10 · 세로 중앙) + 등급 **텍스트형**(라벨 Regular 16 #7D7D80 + 등급만 Bold #041D30, baseline 정렬) — 카드 내 gpill 칩 미사용. ※둥근 박스 반복으로 내용이 안 읽힌다는 피드백에 따른 개편
- **질문 근거 (옛 'AI 인터뷰' 코멘트)**: 라벨 **「질문 근거 | 」** Regular #7D7D80(구분 기호 ' | ' **전후 공백 1칸**) / 면 **bg #F4F4F4 · radius 0** · padding 10px 12px · Regular 14 — '이 근거로 아래 질문을 한다'를 알리는 면접관 가이드 역할(#F8F9FA는 인쇄에서 안 보일 수 있어 상향)
- **질문 행**: **「Q.」 유지**(질문 앞 칩 금지) Bold 16 + 유형 태그(경험 검증·자기 인식 등)는 **질문 뒤** 꼬리 태그(h18 / r3 / bg #F0F0F0 / #696969 / Bold 12 / margin-left 6)
- **답변 예시**: 둥근 박스 없이 불렛 — 「바람직한 답변 | 」「부족한 답변 | 」 라벨 **Bold #7D7D80**(2026-08-03 변경 — 라벨은 물러나고 답변 내용이 앞으로. 전 결과지 공통)
...
- **범례 패널 (core-guide)**: 핵심 역량 섹션 뒤쪽 — bg #F4F4F4 / r8 / padding 16 18 / margin 16 0 / **1단 리스트** = 카드 칩(catchip, min-width 128 중앙정렬) + 설명 Regular 14 #041D30 한 줄
- **가이드 안내문 위치(공통, 2026-08-03)**: 각 섹션의 **맨 뒤** + 아래 여백 **40px**. 핵심 역량 섹션은 **범례 패널 뒤**가 진짜 맨 뒤 (설명문을 먼저 읽고 다시 위로 올라가야 하는 동선 제거)
...
> ## ── 이하 역량검사 컴포넌트 ──
> 아래는 역량검사 결과지의 고유 영역(**주 사용 제품: 역량검사**, 참고). 다른 제품에서도 재사용할 수 있다. source of truth: 역검 v4 결과지 PDF(FAIL 케이스 — **최신 기준**) · Figma 「평가용 역량검사」 PASS 1p(node 7443-2960, PASS 색상만 참조). 스토리북 실물 데모: index.html `#yeokgeom`.
> 페이지 구성: **P1** 지원자 카드+판정 패널·응답 신뢰도·직무/조직 적합도 요약·종합 코멘트 / **P2** 부적응 가능성 상세 / **P3** 업무 역량 종합 점수 / **P4** 직무·조직 적합도 상세 / **P5** 면접 질문. **PASS·FAIL 간 2~5페이지 내용은 동일**하며, 판정에 따라 달라지는 것은 P1 판정 패널뿐이다.

### 업무 역량 종합 판정 패널 (P1 우측)
- **주 사용 제품:** 역량검사
- 패널: width 340px / border-radius 16px / padding 18px 16px — **PASS `linear-gradient(180deg,#2B89F7,#1677E8)` / FAIL `#FF3B30`** (판정에 따라 패널 전체 색 전환)
- 제목 「업무 역량 종합 판정」 Bold 15px #fff 중앙 → 판정 `PASS`/`FAIL` Bold 40px #fff 중앙
- 흰 박스 ①(radius 10 / padding 16px 14px) = **역량 4행 고정**(인지 역량 · 태도 역량 · 협업 역량 · 부적응 가능성)
  - 행: 라벨 Bold 14px(width 86) + 트랙(height 20 / bg #E9E9EA / radius 10) + 점수 칩(height 20 / radius 10 / Bold 12 #fff — 적격 #0075FF · 부적격 #FF3B30 · 주의 #FF7A00) + 판정 배지(width 56 / height 24 / radius 6 / outline — 적격 #0075FF · 부적격 #FF3B30)
- 흰 박스 ②(radius 10 / padding 14px) = **부적응 가능성 요약** — 제목 Bold 15px 중앙 + 하단 구분선 1px #E3E3E3 / 좌측 판정(아이콘 + Bold 16px, 색 = 최고 위험도) / 우측 3행(갈등 유발·불성실 근무·조기 퇴사 가능성) + 배지(width 48 / height 22 / radius 4 / outline)
- ⚠ **종합 판정과 개별 적격 판정은 별개 로직** — 종합 PASS라도 개별 항목은 부적격일 수 있다(임의로 일치시키지 말 것)

### 응답 신뢰도 패널 (P1 좌측)
- **주 사용 제품:** 역량검사
- 카드: border 1px #E3E3E3 / radius 8 / padding 18px 20px — 제목 Bold 16px → 판정 행 → 서술(Regular 13px / line-height 1.7)
- 판정: 아이콘(원형 20px 채움) + Bold 18px, 색 = 최고 위험도
- 지표 **4종 고정 순서**: 무응답 · 무성의 · 과장 · 비전형 — 라벨 Bold 13px + 상태 배지
- 상태 배지 3단계(width 44 / height 22 / radius 4 / outline / Bold 12px): **양호 #0075FF · 주의 #FF7A00 · 위험 #FF3B30**
- ⚠ 역량검사의 3단 신호는 **파랑/주황/빨강** 체계 — 초록(#20AA40) 미사용

### 직무 적합도 (Job Fit)
- **주 사용 제품:** 역량검사
- 좌: 직무명 Bold 22px #0075FF + **반원 게이지**(SVG arc / stroke-width 16 / 트랙 #E9E9EA / 값 #0075FF / round cap) + 중앙 % Bold 30px + 해시태그 역량 3종(13px #0075FF)
- 우: 주요 직무 역량 3행 — 이름 Bold 14px + 점수 Bold 14px #0075FF + 막대(height 8 / radius 4 / 트랙 #E9E9EA)
- P4 상세에는 **매칭 직무 TOP 3**(순위 + 직무명 + 해시태그 역량) 동반

### 조직 적합도 (Culture Fit)
- **주 사용 제품:** 역량검사
- 유형 헤드: bg **#E9F3FF** / radius 6 / padding 12px 16px — 유형명 Bold 18px #0075FF + 한 줄 요약 Bold 14px #041D30 (baseline 정렬)
- 속성 **3행 고정**: 핵심가치 · 적합문화 · 부적합문화 — 라벨 Bold 14px(width 80) + 값 Regular 14px / line-height 1.6
- P4 상세에는 **직업가치관 2×4 그리드**(8유형 중 선택 유형만 테마색 채움) + **조직 내 예측 행동 4카드**(상황 제목 + 서술, 2×2) 동반
- ※ 명칭은 「조직 적합도」 (Figma의 「직업 가치관」·동기부여 포인트 항목은 구버전)

### 부적응 · 리스크 지표 (P2)
- **주 사용 제품:** 역량검사
- **3영역 고정**: 갈등 유발 · 불성실 근무 · 조기 퇴사 가능성 — 영역명 Bold 14px + 대표 점수 Bold 20px + 상태 배지(width 40 / height 20 / radius 4 / outline)
- 세부 지표 막대: **3구간 트랙**(양호 #E9E9EA / 주의 #FFF4E0 / 위험 #FFEAF0 · 균등 3분할 · radius 2) + 값 막대(height 4) + 값 칩(height 18 / radius 9 / 흰 배경 outline) · 트랙 하단에 양호/주의/위험 구간 라벨
- 값 색 = 해당 구간 색: **양호 #0075FF · 주의 #FF7A00 · 위험 #FF006B** / 영역 대표 상태 배지의 위험은 #FF3B30
- 요약은 **최고 위험도를 대표값**으로 (P1 부적응 가능성 요약과 동일 규칙)

## 제품별 색상 테마
> 제품군 정의는 위 '제품군 표기 규칙' 참조.

- 몬스터: #6B1FCC → #9330FF (그라디언트)
- 역량검사: #08174A (구 추정값) — Figma 「평가용 역량검사」 실측 **#002263** (지원자 카드 네이비). 어느 쪽이 정본인지 개발팀 확인 필요
